...
const createVital = async (req, res) => {
  try {
    ...
    const vital = await Vital.findOne({ where: { name } });
    if (vital) {
      return res
        .status(403)
        .json({ status: "fail", message: "Vital Sign already exists" });
    }
...
const getVital = async (req, res) => {
  try {
    ...
    const vital = await Vital.findOne({ where: { uuid: uuid },include:['vitalValue'] });
...
const updateVital = async (req, res) => {
  try {
    ...
    if (!vital) {
      return res
        .status(404)
        .json({ message: "No vital sign found with that ID" });
    }
